...
const TripResults = ({ tripData, onEdit, onSave }) => {
  ...
  const handleEvaluatePlan = async () => {
    ...
    setEvaluating(true);
    try {
      const response = await fetch('http://localhost:5000/evaluate-plan', {
        method: 'POST',
    ...
    <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          {typeof tripData.plan === 'object' && tripData.plan.tripPlan ? (
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-6">
              <div className="flex justify-between items-center mb-3">
                <h3 className="font-semibold text-amber-900">🤖 AI-Generated Itinerary</h3>
                <button
      ...
      {evaluation && (
        <div className="mt-6 bg-green-50 border border-green-200 rounded-lg p-6">
          <h3 className="font-semibold text-green-900 mb-4">📊 Plan Evaluation Results</h3>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Heuristics Score */}
            <div>
              <h4 className="font-medium text-green-800 mb-2">Heuristic Analysis</h4>
              <div className="space-y-2">
                <div className="flex justify-between">
                  <span className="text-sm">Overall Score:</span>
                  <span className="font-semibold">{Math.round(evaluation.heuristics.score * 100)}%</span>
                </div>
                ...
                <div className="text-xs text-green-700">
                  Structure: {Math.round(evaluation.heuristics.breakdown.structure * 100)}% | 
                  Detail: {Math.round(evaluation.heuristics.breakdown.detail * 100)}% | 
                  Practicality: {Math.round(evaluation.heuristics.breakdown.practicality * 100)}% | 
                  Tips: {Math.round(evaluation.heuristics.breakdown.tips * 100)}%
                </div>
              </div>
            </div>
              ...
              <div className="text-sm text-green-700">
                {(() => {
                  try {
                    ...
                    return (
                      <div className="space-y-2">
                        <div><strong>Score:</strong> {Math.round(critique.overallScore * 100)}%</div>
                        {critique.strengths?.length > 0 && (
                          <div><strong>Strengths:</strong> {critique.strengths.join(', ')}</div>
                        )}
                        {critique.suggestions?.length > 0 && (
                          <div><strong>Suggestions:</strong> {critique.suggestions.join(', ')}</div>
                        )}
                      </div>
                    );
                  } catch (e) {
                    return <div className="text-xs">Critique parsing failed</div>;
                  }
                })()}
              </div>
            </div>
